...
ax.set_ylabel('Torso Roll Angle\nVariability (rad)')
...
ax.bar(x_pos, np.median(data['torso_std_y'],0), color = bar_color,
    tick_label=('No Pros','NM','IMP','No Pros','NM','IMP'))
add_barplot_sigstars(ax, data['condition_combinations']-1, 
    data['p_values_torso_std_y'], x_pos, star_loc='3x3')
...
fig.savefig('treadmill_vib_torso_var_y.pdf', bbox_inches='tight')
plt.close(fig)
