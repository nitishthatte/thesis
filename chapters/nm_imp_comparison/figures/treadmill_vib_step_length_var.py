...
ax.bar(x_pos, np.mean(data['step_length_var'],0), color = bar_color,
    tick_label=('No Pros','NM','IMP','No Pros','NM','IMP'))
add_barplot_sigstars(ax, data['condition_combinations']-1, 
    data['p_values_step_length_var'], x_pos, star_loc='3x3')

subject_list = range(num_subjects)
scatter_opts = {'s':10, 'zorder':10,'marker':'o'}
for i in range(6):
    ax.scatter(x_pos[i] + rand_scatter_pts, data['step_length_var'][:,i], 
...
fig.savefig('treadmill_vib_step_length_var.pdf', bbox_inches='tight')
plt.close(fig)
