...
those using ground reaction signals fo the transitions between phases
...
pregenerated discrete set of possiblities. It is possible and likely that there
...
However, this approach is still insufficent for impedance control of both the
...
comfort. In this expeirment, we allowed users to specify their own objective by
using their preferences. In future work, similar prefence based approaches could
...
features we would use to quanity gait are difficult to measure. For example, to
measure gait robustness using treadill velocity disturbances, we required many
...
quickly measureable stand-ins for these metrics should be a focus of future
research.
