...
data = sio.loadmat('prob_landing_angle_data.mat')
...
ax[1].set_xlabel('Landing Knee Flexion Angle (deg)')
ax[1].set_ylabel('Probability of Knee Flexion Angle')
...
minjerk_handle = ax[0].fill_between(x_pts, 0, y_pts_minjerk, color = colors[0,:],
    linewidth=0)
mpc_handle = ax[0].fill_between(x_pts, 0, y_pts_mpc, color = colors[1,:], linewidth=0)
...
ax[0].set_ylim((0.095, 0.14))
ax[0].spines['left'].set_bounds(0.09, 0.14)
ax[0].set_yticks(np.arange(0.1,0.16, 0.02))
ax[0].set_xlim((-4, 90))
...
ax[1].set_yticks(np.arange(0.,0.015, 0.005))
ax[1].spines['bottom'].set_bounds(0, 90)
ax[1].set_xticks(np.arange(0,120,30))
...
fig.suptitle('Propensity for Tripping', fontweight=title_font_weight)
...
fig.savefig('prob_landing_angles.pdf', bbox_inches='tight')
plt.close(fig)
