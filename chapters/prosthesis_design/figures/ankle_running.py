...
p2, = ax.plot([0, max_speed], [tau_rated, tau_rated], '--',
    linewidth=2, color=color2)
p3, = ax.plot([0, max_speed], [torque_motor_rms, torque_motor_rms], '--',
    linewidth=2, color=color3)

ax.legend((p0, p1, p2, p3), ('Motor Torque', 'Torque Limit', 
    'Rated Torque', 'Motor RMS Torque'), frameon = False, loc = (-0.1, -1),
    handlelength=3)
...
ax.set_yticks((0, np.round(torque_motor_rms,2), tau_rated, tau_max))
